##most recent working code, AGR 082724 @ 7:30pm EST
library(shiny)
library(plotly)
library(DT)
library(dplyr)
...
check_command <- function(command) {
  if (Sys.which(command) == "") {
    stop(paste(command, "is not installed or not in the system PATH. Please install it manually."))
  }
}

# Check for 'git' and 'gcc' availability
check_command("git")
check_command("gcc")
...
ui <- fluidPage(
    ...
    tabPanel("Run and Process",
             sidebarLayout(
               sidebarPanel(
                 ...
                 textInput("input_file", "Enter the path to a FASTA File", value = "Path/to/file/yourfile.fasta"),
             ...
             fluidRow(
               ...
               column(9,
                      box(
                        ...
                        plotOutput("tree", width = "100%", height = "600px"),  # Set a fixed height
  ...
  tags$style(
    type = "text/css",
    "#tabs li:nth-child(2) a[data-toggle='tab'] {width: 100%;}"
  )
)
...
server <- function(input, output, session) {
  ...
  run_zhunt <- function(input_file, params) {
    preprocess_fasta(input_file)
    command <- paste(Z_path, params, input_file)
  ...
  observeEvent(input$fetch, {
    ...
    updateTextInput(session, "input_file", value = normalizePath(modified_fasta_file))
  })


  observeEvent(input$run, {
    req(input$input_file)
    modified_fasta_file <- normalizePath(input$input_file)
    original_fasta_file <- paste0(tools::file_path_sans_ext(modified_fasta_file), ".fasta")
    params <- input$params
    command_output <- run_zhunt(modified_fasta_file, params)
  ...
  observeEvent(input$process, {
    ...
    zscore_file <- paste0(modified_fasta_file, ".Z-SCORE")
...
c1_ol = ol
      ...
      output$download <- downloadHandler(
        filename = function(){"Filtered_Z_GENIE_output.csv"},
  ...
  output$plot <- renderPlotly({
    req(data())
    plot_ly(data(), x = ~get(input$x, data()), y = ~get(input$y, data()), color = ~get(input$color, data()), type = 'scatter', mode = 'markers', text = ~paste(
      ...
      "<br>Palindromic Sequence: ", data()$palindromic_sequence
                  ...
                  xaxis = list(range = c(min(data()[[input$x]]), max(data()[[input$x]]))),
                  yaxis = list(range = c(min(data()[[input$y]]), max(data()[[input$y]])))
    )
  })
  ...
  observe({
    ...
    if (!is.null(input$table_rows_all)) {
      ...
      output$plot <- renderPlotly({
        req(filtered_data)
        plot_ly(filtered_data, x = ~get(input$x, filtered_data), y = ~get(input$y, filtered_data), color = ~get(input$color, filtered_data), type = 'scatter', mode = 'markers', text = ~paste(
  ...
  observe({
    ...
    if (!is.null(input$table_rows_all)) {
      ...
      output$download2 <- downloadHandler(
        filename = function(){"Filtered_Z_GENIE_Uploaded_Data.csv"},
        content = function(fname){
