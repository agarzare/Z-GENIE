         ...
         tabItem(tabName = "run_process", fluidRow(
            box(
               ...
               conditionalPanel(
                  ...
                  actionButton("manual_run", "Run Z-Hunt"),
               ...
               conditionalPanel(
                  ...
                  textInput("params_upload", "Z-Hunt Parameters", "8, 6, 8"),
                  actionButton("run_upload", "Run Z-Hunt"),
                  numericInput("zscore_threshold_upload", "Manual Z-Score Threshold", 600)
               ),
...
server <- function(input, output, session) {
   ...
   #TIMINGTEST START : initializing the data frame to be saved on local device
   # remember to change the type here.
   # The options for the type of run are: palindrome, ISS, or both
   # now he also wants the Zp value
   zp_path <- "zp_vals_try_6.csv"
   test_path <- "testing_both_try_6.csv"
   cur_run <- "both"
   
   zp_df <-  data.frame(nuc_ID = character(), zp = character(), run_type = character(), stringsAsFactors = FALSE)
  ...
  if (!file.exists(test_path)) {
    write.csv(timing_df, test_path, row.names = FALSE)
  } 
   
   if (!file.exists(zp_path)) {
     write.csv(zp_df, zp_path, row.names = FALSE)
   } 
   ...
   observeEvent(input$fetch, {
      withProgress(message = "Fetching FASTA from NCBI...", value = 0, {
         ...
         total_time <- difftime(time_end, time_start, units = "secs")
           # round(as.numeric(difftime(time_end, time_start, units = "secs")), 2)
         showNotification(paste("Fetch completed in", total_time, "seconds."),
                          type = "message",
                          duration = 10)

         #TIMINGTEST 1: Timing the fetch, save to dataframe
         new_entry <- data.frame(nuc_id = input$nucleotide_id, step = "fetch", time = total_time, run_type = cur_run, stringsAsFactors = FALSE)
         write.table(new_entry, test_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
   ...
   observeEvent(input$run, {
      withProgress(message = "Running Z-Hunt...", value = 0, {
         ...
         total_time <- difftime(time_end, time_start, units = "secs")
         #total_time <- round(as.numeric(difftime(time_end, time_start, units = "secs")), 2)
         showNotification(paste("Z-Hunt completed in", total_time, "seconds."),
                          type = "message",
                          duration = 10 )

         #TIMINGTEST 2: timing z hunt run
         new_entry <- data.frame(nuc_id = input$nucleotide_id, step = "zhunt", time = total_time, run_type = cur_run, stringsAsFactors = FALSE)
         write.table(new_entry, test_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
   ...
   observeEvent(input$process, {
      withProgress(message = "Processing Z-SCORE File (FULL)", value = 0, {
         ...
         # TIMINGTEST: timings for the ISS pal or both START
         ...
         # TIMINGTEST: timings for the ISS pal or both END
         ...
         #TimingTest 2.5: save result in thing
         new_entry <- data.frame(nuc_id = input$nucleotide_id, zp = Zp, run_type = cur_run, stringsAsFactors = FALSE)
         write.table(new_entry, zp_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
         ...
         new_entry <- data.frame(nuc_id = input$nucleotide_id, step = "zscoreproc", time = total_time, run_type = cur_run, z_thresh = threshold, stringsAsFactors = FALSE)
         write.table(new_entry, test_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
   ...
   observeEvent(input$generate_bed, {
      withProgress(message = "Generating BED File (Minimal)", value = 0, {
         ...
         #TIMINGTEST 4: How long does this take
         new_entry <- data.frame(nuc_id = input$nucleotide_id, step = "bedfile", time = total_time, run_type = cur_run, stringsAsFactors = FALSE)
         write.table(new_entry, test_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
