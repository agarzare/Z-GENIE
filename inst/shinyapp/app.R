...
server <- function(input, output, session){
  ...
  #TIMINGTEST : initializing the data frame 
  # remember to change the type here.
  test_path <- "testing_original.csv"
  
  cur_run <- "original"
  timing_df <- data.frame(nuc_id = character(), step = character(), time = character(), run_type = character(), stringsAsFactors = FALSE)
  ...
  observeEvent(input$create_fasta,{
    withProgress(message="Creating FASTA from typed sequence...", value=0, {
      ...
      #TIMINGTEST 0 : Not needed, only timing fetch, which we assume is worst case
  ...
  observeEvent(input$process,{
    withProgress(message="Processing Z-SCORE File (FULL)", value=0, {
      ...
      new_entry <- data.frame(nuc_id = input$nucleotide_id, step = "zscoreproc", time = total_time, run_type = cur_run, stringsAsFactors = FALSE)
      write.table(new_entry, test_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE)
