...
# -------------------------------
df1 <- read.csv("./data/benchmarks/cleaned_output_ISS_Pal.csv", stringsAsFactors = FALSE)
df2 <- read.csv("./data/benchmarks/real_samples.csv", stringsAsFactors = FALSE)
...
#Correcting NA from Figure 4; NC_009457.1 needs manual curation -- updated 09/22/2025
NC_009457.1 <- benchmark_data$nuc_ID == "NC_009457.1"
benchmark_data$desc[NC_009457.1] <- "Vibrio cholerae O395 chromosome 1, complete sequence"
benchmark_data$genome_size[NC_009457.1] <- as.numeric(3024069)
...
write.csv(benchmark_data, 
          "./data/benchmarks/benchmark_data.csv", 
          row.names = FALSE)
```
...
p_total <- ggplot(total_times, aes(x = genome_size, y = total_time, color = size_category)) +
  ...
  scale_y_continuous(labels = comma, expand = expansion(mult = c(0.05, 0.05))) +
  labs(x = "Genome Size (bp)",
       y = "Total Processing Time (seconds)",
       color = "Genome Size Category") +
  theme_minimal(base_size = 16) +
  theme(
    axis.title = element_text(face = "bold", size = 18),
...
ggsave(
  filename = "./data/benchmarks/Figure_4.tiff",
  plot = final_plot,
  width = 7.2, 
  height = 10, 
  units = "in",
